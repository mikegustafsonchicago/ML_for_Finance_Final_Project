...
from Utils.latex_utility import save_latex_file, latex_section, latex_subsection, latex_list  # Add this import
...
if __name__ == "__main__":
    ...
    try:
        latex_content = latex_section(
            "Temperature Analysis: Individual Airports",
            "Each plot shows temperature trends for a single regional airport, year-over-year. "
            "Temperature data is shown over the course of a year, with different colors representing different years. "
            "A second plot for each airport shows only the week of April 1-7 as an example of short-term variability."
        )
        airport_names = [airport for airport, _, _ in plot_paths]
        latex_content += latex_subsection(
            "Airports Included",
            latex_list(airport_names)
        )
        for airport, plot_path_full, plot_path_week in plot_paths:
            latex_content += latex_subsection(
                f"{airport}",
                ...
                \\begin{{figure}}[htbp]
                \\centering
                \\includegraphics[width=0.7\\textwidth]{{{plot_path_full.name}}}
                \\caption{{Temperature Over Time: {airport}}}
                \\label{{fig:{airport.lower()}_full_year}}
                \\end{{figure}}
                \\begin{{figure}}[htbp]
                \\centering
                \\includegraphics[width=0.7\\textwidth]{{{plot_path_week.name}}}
                \\caption{{Temperature: Example Week (April 1-7) - {airport}}}
                \\label{{fig:{airport.lower()}_april1week}}
                \\end{{figure}}
                """
            )
        latex_output_path = output_dir / 'temperature_analysis.tex'
        save_latex_file(latex_content, latex_output_path)
        logger.info(f"LaTeX temperature analysis report created: {latex_output_path}")
    except Exception as e:
        logger.error(f"Error creating LaTeX temperature report: {str(e)}")
        logger.debug("Error details:", exc_info=True)
        raise
